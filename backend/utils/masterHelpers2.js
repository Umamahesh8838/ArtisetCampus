...
async function resolveSkillId(connection, skillName) {
  return await resolveGenericId('tbl_cp_mskills', 'skill_id', 'name', skillName, connection);
}
...
async function resolveCertificationId(connection, certName, issuer) {
  ...
  const nid = await insertWithNextId(connection, 'tbl_cp_mcertifications', 'certification_id', ['certification_name','issuing_organization'], [valStr, issuer || 'Unknown']);
  return nid;
}
