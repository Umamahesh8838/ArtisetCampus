...
async function run() {
  ...
  try {
    ...
    userId = ins.insertId;
    console.log('Created test user id=', userId, email, phone);
  ...
  const r = await fetch(`http://${host}:${process.env.PORT || 3000}/auth/me`, { method: 'GET', headers: { Authorization: 'Bearer ' + token } });
    ...
    console.log('Registration submit response status=', res.status);
    ...
    console.log('tbl_cp_student rows for user:', stuRows.length);
    ...
    console.log('Cleaned up test records for user', userId);

  } catch (err) {
    console.error('test_e2e error:', err);
    if (userId) {
      try {
